...
class LSViTBackbone(nn.Module):
    ...
    def forward(self, video: torch.Tensor) -> torch.Tensor:
        """Extract frame-wise token features from a video.

        Args:
            video: Input video tensor of shape `(B, T, C, H, W)`.

        Returns:
            Feature tensor of shape `(B, T, N+1, C)`.
...
class LSViTForAction(nn.Module):
    ...
    def __init__(
        ...
        Args:
            config: Model configuration.
            num_classes: Optional override for number of classes.
            smif_window: Optional override for SMIF temporal window.
        """
        super().__init__()
    ...
    def forward_features(self, video: torch.Tensor) -> torch.Tensor:
        """Extract pooled video features before classification.

        Args:
            video: Input tensor of shape `(B, T, C, H, W)`.
